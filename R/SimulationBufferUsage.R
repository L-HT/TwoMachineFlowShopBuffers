...
#' @export
getCurrentBufferUsageR <- function(jobData, permutationM1, permutationM2, maxBufferSize){
  m1 <- Machine$new()
  m2 <- Machine$new()
  m1$name <- "m1"
  m2$name <- "m2"
...
#' @export
getCurrentBufferUsageTotalBufferR <- function(jobData, permutationM1, permutationM2, maxBufferSize){
  m1 <- Machine$new()
  m2 <- Machine$new()
  m1$name <- "m1"
  m2$name <- "m2"
  ...
  print("ende")
  return(buffer)#sum(buffer$bufferUsage))
}
