...
int simulateFlowShopC_DueTime(Rcpp::DataFrame jobDataDF, std::vector<std::string> permutationM1,
  ...
  Rcpp::IntegerVector dueTimes = jobDataDF["dueTime"];
  ...
  while(!finished){
    ...
    if (m2Event.state_ == "end"){
      ...
      // fertiggestellten Job suchen
      int indexOfFinishedJob = -1;
      int tempCounter = 0;
      while (indexOfFinishedJob == -1){
        if (m2Event.jobName_ == std::string(jobNames[tempCounter])){
          indexOfFinishedJob = tempCounter;
        } else {
          tempCounter++;
          if (tempCounter >= 2*jobDataDF.nrow()){
            Rcpp::stop("Suche nach fertigem M2-Job fehlgeschlagen");
          }
        }
      }

      int timeDifference = (time + 1) - dueTimes[tempCounter];
      if (timeDifference > 0){
