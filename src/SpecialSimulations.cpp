  ...
  while(!finished){
    ...
    if (!jobsJustFinished.empty()){
      if (nextJobName == jobsJustFinished[0].name_ && !m2.busy_){
        ...
        if (m1.blocked_){
          m1.blocked_ = false;
          // hier Return, falls M1 fertig
          if (nextJobM1 >= permutationM1.size() && !getMakespan){
        ...
        if (currentBufferUsage + justFinishedJobsBufferUsage <= maxBufferSize){
          ...
          //----------hier return Benutzung des Speichers
          if (nextJobM1 >= permutationM1.size() && !getMakespan){
            return currentBufferUsage + justFinishedJobsBufferUsage;
          }
          m1.busy_ = false;
          if (m1.blocked_){
            m1.blocked_ = false;
